...
const Tools = {
    ...
    DRAW_MAP_PNG: function (options, callback) {
        ...
        const BOUNDS = {
            x1: Math.min(Math.max(0,settings.crop_x1), Math.max(0,options.parsedMapData.image.dimensions.width-1)),
            x2: Math.min(Math.max(0,settings.crop_x2), options.parsedMapData.image.dimensions.width),
            y1: Math.min(Math.max(0,settings.crop_y1), Math.max(0,options.parsedMapData.image.dimensions.height-1)),
            y2: Math.min(Math.max(0,settings.crop_y2), options.parsedMapData.image.dimensions.height),
        };
